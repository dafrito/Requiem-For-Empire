...
class CompileThread extends Thread{
	...
	public void run(){
		...
		if(!quickflag){m_debugEnvironment.setStatus("One or more files had errors during compilation.");return;}
...
public class Debug_Environment extends JFrame implements ActionListener,ChangeListener{
	...
	private JMenuItem m_newFile,m_openFile,m_closeFile,m_saveFile,m_saveFileAs,m_exit,m_reset,m_report;
	...
	private JMenuItem m_addException,m_addExceptionFromList,m_addIgnore,m_addIgnoreFromList,m_removeException,m_removeIgnore;
	...
	public boolean isInExceptionsMode(){return m_exceptionsMode.isSelected();}
	public java.util.List<Debug_ScriptElement>getScriptElements(){return m_scriptElements;}
	...
	public void canExecute(boolean value){m_execute.setEnabled(value);}
	...
	public boolean ensureCurrentNode(Object obj){
		if(!m_reset.isEnabled()){return true;}
		if(isInExceptionsMode()&&!m_exceptions.contains(Thread.currentThread().getName())){return true;}
		if(m_ignores.contains(Thread.currentThread().getName())){return true;}
	...
	public boolean isIgnoringThisThread(){
		...
		if(isInExceptionsMode()){
			for(String thread:m_exceptions){
				if(currentThread.contains(thread)){return false;}
			}
			return true;
		}else{
			for(String thread:m_ignores){
				if(currentThread.contains(thread)){return true;}
			}
			return false;
		}
	}
	...
	public void addNode(Debug_TreeNode node,boolean setAsCurrent){
		if(isIgnoringThisThread()){return;}
		...
		for(Debug_Listener listener:m_filteredOutputMap.get(Thread.currentThread().getName())){
			if(listener.isCapturing()){
				...
				}else{addNodeToOutput(node,setAsCurrent,listener);}
				return;
			}
		}
		for(Debug_Listener listener:m_filteredOutputMap.get(Thread.currentThread().getName())){
	...
	private void addNodeToOutput(Debug_TreeNode node, boolean setAsCurrent,Debug_Listener listener){
		if(!listener.getTreePanel().getFilter().isListening()){
			listener.getTreePanel().getFilter().sniffNode(node);
			if(!listener.getTreePanel().getFilter().isListening()){return;}
		...
		if(listener.getTreePanel().getCurrentNode().hasChildren()&&listener.getTreePanel().getCurrentNode().getLastChild().getGroup()!=null&&node.getGroup()!=null&&listener.getTreePanel().getCurrentNode().getLastChild().getGroup().equals(node.getGroup())){
			if(listener.getTreePanel().getCurrentNode().getLastChild().getData().equals(node.getGroup())){
				...
				if(setAsCurrent){listener.getTreePanel().setAsCurrent(node);}
			}else{
	...
	public Debug_Listener getUnfilteredOutput(){return m_filteredOutputMap.get(Thread.currentThread().getName()).get(0);}
	public Debug_TreeNode getLastNodeAdded(){return getUnfilteredOutput().getTreePanel().getLastNodeAdded();}
	// Deprecated, deprecated, deprecated...
	public boolean printDebug(String category, Object obj){return true;}
	// Listeners
	...
	public boolean isResetting(){return !m_reset.isEnabled();}
...
class Debugger{
	...
	// Noding functions.
	public static boolean openNode(Object string){openNode(null,string);return true;}
	public static boolean openNode(Object group,Object string){
		if(getDebugger().isIgnoringThisThread()){return true;}
		getDebugger().openNode(new Debug_TreeNode(group,string));return true;
	}
	public static boolean addSnapNode(Object name, Object o){return addSnapNode(null,name,o);}
	...
	public static String getString(DebugString value){return Debug_TreeNode.getPrecached(value).toString();}
	...
	public static void setExceptionsMode(boolean value){getDebugger().setExceptionsMode(value);}
	public static boolean isResetting(){return getDebugger().isResetting();}
	...
	public static void reset(){getDebugger().reset();}
