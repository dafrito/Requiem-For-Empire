...
class StylesheetParams{
	...
	public List<Object> getModifiers(){return m_modifiers;}
	public Referenced getDebugReference(){return m_reference;}
}
...
public class Parser{
		...
		for(int i=0;i<lineList.size();i++){
			...
			if(element instanceof ScriptKeyword){
				if(element.equals(ScriptKeywordType.CLASS)){
					...
					String name=((ScriptLine)lineList.get(i)).getString();i++;
					...
					do{
						...
						if(lineList.get(i)instanceof ScriptGroup){
							body=(ScriptGroup)lineList.get(i);break;
						}
						modifiers.add(lineList.get(i));i++;
	...
	public static ScriptTemplate_Abstract preparseTemplate(Referenced ref, ScriptEnvironment env, List<Object>modifiers, ScriptGroup body,String className)throws Exception_Nodeable{
		...
		for(int i=0;i<modifiers.size();i++){
			...
			if(modifiers.get(i)instanceof ScriptKeyword){
				if(modifiers.get(i).equals(ScriptKeywordType.EXTENDS)){
					...
					modifiers.remove(i);modifiers.remove(i);i--;
				}else if(modifiers.get(i).equals(ScriptKeywordType.IMPLEMENTS)){
					if(i==modifiers.size()-1){throw new Exception_Nodeable_UnexpectedType(env,modifiers.get(i),"Interfaces");}
					boolean flag=false;
					while(i<modifiers.size()){
						flag=true;
						modifiers.remove(i);
						if(!(modifiers.get(i)instanceof ScriptLine)){throw new Exception_Nodeable_UnexpectedType(env,modifiers.get(i),"Interface type");}
		...
		if(extendedClass==null||extendedClass.equals("")){extendedClass=FauxTemplate_Object.OBJECTSTRING;}
		...
		for(int i=0;i<list.size();i++){
			...
			if(list.get(i)instanceof ScriptGroup){
				if(stylesheet){
					...
					parseExpression(env,expressionList,true,ScriptValueType.createType(env,className));
					elements.clear();stylesheet=false;continue;
				}
	...
	public static ScriptExecutable_ParseFunction preparseFunction(ScriptEnvironment env, ScriptTemplate_Abstract object,List<Object>modifiers,ScriptGroup paramGroup,ScriptGroup body,String name) throws Exception_Nodeable{
		if(name.equals("")){assert Debugger.openNode("Preparsing Functions","Preparsing Function (constructor)");
		}else{assert Debugger.openNode("Preparsing Functions","Preparsing Function ("+name+")");}
		if(object!=null){assert Debugger.addSnapNode("Reference Template",object);}
		...
		for(int i=0;i<modifiers.size();i++){
			if(modifiers.get(i)instanceof ScriptKeyword){
				if(ref==null){ref=(ScriptKeyword)modifiers.get(i);}
				if(i==modifiers.size()-1){throw new Exception_Nodeable_UnexpectedType((Referenced)modifiers.get(i),"Function name");}
				...
				if(keyword.equals(ScriptKeywordType.STATIC)){
					...
					}else{throw new Exception_Nodeable_UnexpectedType((Referenced)modifiers.get(i),"Modifier");}
				}
			}
			if(modifiers.get(i)instanceof ScriptLine){
				if(ref==null){ref=(ScriptLine)modifiers.get(i);}
				if(i==modifiers.size()-1){
					// It's a function name
					if(returnType!=null){functionName=((ScriptLine)modifiers.get(i)).getString();break;}
					// It's a constructor
					if(returnType==null){returnType=ScriptValueType.createType((ScriptLine)modifiers.get(i),((ScriptLine)modifiers.get(i)).getString());break;}
				}
				if(i!=modifiers.size()-2){throw new Exception_Nodeable_UnexpectedType((Referenced)modifiers.get(i),"Modifier");}
				if(!(modifiers.get(modifiers.size()-1)instanceof ScriptLine)){throw new Exception_Nodeable_UnexpectedType((Referenced)modifiers.get(modifiers.size()-1),"Function name");}
				if(returnType!=null){throw new Exception_Nodeable_UnexpectedType((Referenced)modifiers.get(i),"Modifier");}
	...
	public static ScriptExecutable parseFlowElement(ScriptEnvironment env,List<Object>list,ScriptValueType type) throws Exception_Nodeable{
		...
		for(int i=0;i<list.size();i++){
			...
			if(obj instanceof ScriptExecutable){continue;}
			if(obj instanceof ScriptKeyword){
				if(obj.equals(ScriptKeywordType.FOR)){
					env.advanceNestedStack();
					list.remove(i);
					if(!(list.get(i)instanceof ScriptGroup)){throw new Exception_Nodeable_UnexpectedType(env,list.get(i),"Param group");}
					List<ScriptExecutable>parameterList=parseBodyList(env,((ScriptGroup)list.get(i)).getElements(),type);
					if(parameterList.size()!=3){throw new Exception_Nodeable_UnexpectedType(env,list.get(i),"'for' statement parameters");}
					list.remove(i);
					if(!(list.get(i)instanceof ScriptGroup)){throw new Exception_Nodeable_UnexpectedType(env,list.get(i),"Curly group");}
				...
				if(obj.equals(ScriptKeywordType.ELSE)){
					...
					ScriptExecutable_IfStatement previous=(ScriptExecutable_IfStatement)list.get(i);list.remove(i);
					if(list.get(i)instanceof ScriptKeyword&&list.get(i).equals(ScriptKeywordType.IF)){
						Referenced ref=(Referenced)list.get(i);list.remove(i);
	...
	public static List<ScriptExecutable>parseBodyList(ScriptEnvironment env,List<Object>bodyElements,ScriptValueType type)throws Exception_Nodeable{
		...
		for(int j=0;j<bodyElements.size();j++){
			...
			if(bodyElements.get(j)instanceof ScriptGroup&&((ScriptGroup)bodyElements.get(j)).getType()==ScriptGroup.GroupType.curly){
				if(j+1<bodyElements.size()&&bodyElements.get(j+1).equals(ScriptKeywordType.ELSE)){continue;}
				statementBodyList.add(parseFlowElement(env, elements,type));
				elements.clear();continue;
			}
		}
		if(elements.size()!=0){statementBodyList.add(parseExpression(env,elements,false,type));}
		assert Debugger.closeNode();
	...
	public static ScriptExecutable_IfStatement parseIfStatement(Referenced ref, List<Object>list,ScriptValue_Abstract value,int i,ScriptValueType type)throws Exception_Nodeable{
		...
		if(value==null){
			if(!(list.get(i)instanceof ScriptGroup)){throw new Exception_Nodeable_UnexpectedType(ref,list.get(i),"Param group");}
			value=(ScriptValue_Abstract)parseExpression(ref.getEnvironment(),((ScriptGroup)list.get(i)).getElements(),false,type);list.remove(i);
		}
		if(!(list.get(i)instanceof ScriptGroup)){throw new Exception_Nodeable_UnexpectedType(ref,list.get(i),"Curly group");}
	...
	public static Stylesheet parseStylesheet(Referenced ref, ScriptEnvironment env,boolean isUnique, String name,ScriptGroup group) throws Exception_Nodeable{
		...
		for(int i=0;i<elements.size();i++){
			if(elements.get(i)instanceof ScriptOperator&&((ScriptOperator)elements.get(i)).getType()==ScriptOperatorType.SEMICOLON){continue;}
	...
	public static ScriptExecutable parseExpression(ScriptEnvironment env, List<Object>list,boolean automaticallyAddToStack,ScriptValueType type) throws Exception_Nodeable{
		if(list.size()==1&&list.get(0)instanceof ScriptExecutable){return (ScriptExecutable)list.get(0);}
		for(int i=0;i<list.size();i++){
			//assert Debugger.addSnapNode(DebugString.ELEMENTS,list);
			Object obj=list.get(i);
			//assert Debugger.addSnapNode("Current element ("+obj+")",obj);
			Object nextObj=null;
			if(i<list.size()-1){nextObj=list.get(i+1);}
			...
			if(obj instanceof ScriptGroup){
				...
				list.add(i,parseExpression(env,groupList,automaticallyAddToStack,type));continue;
			}
			...
			if(obj instanceof ScriptKeyword&&(((ScriptKeyword)obj).equals(ScriptKeywordType.STYLESHEET)||ScriptValueType.isPrimitiveType(((ScriptKeyword)obj).getValueType()))){
				...
				int loc=i;
				while(loc>0){
					if(!(list.get(loc) instanceof ScriptKeyword)){throw new Exception_Nodeable_UnexpectedType((Referenced)list.get(loc),"Keyword");}
					loc--;
				}
				int temp=i;
				i=loc;
				...
				do{
					if(!(list.get(i+1) instanceof ScriptLine)){throw new Exception_Nodeable_UnexpectedType((Referenced)list.get(i+1),"Variable name");}
					String name=((ScriptLine)list.get(i+1)).getString();
					...
					list.remove(i);list.remove(i);list.add(i,exec);
					if(i<list.size()-1){i++;}
			...
			if(obj.equals(ScriptKeywordType.NEW)){
				if(!(nextObj instanceof ScriptLine)){throw new Exception_Nodeable_UnexpectedType((Referenced)nextObj,"Object name");}
				if(!(list.get(i+2) instanceof ScriptGroup)){throw new Exception_Nodeable_UnexpectedType((Referenced)nextObj,"Parameters");}
				if(env.getTemplate(((ScriptLine)nextObj).getString())==null){throw new Exception_Nodeable_TemplateNotFound((ScriptLine)nextObj,((ScriptLine)nextObj).getString());}
			...
			if(obj instanceof ScriptOperator){
				ScriptValue_Variable lhs;ScriptValue_Abstract left;ScriptExecutable returnValue;
				switch(((ScriptOperator)obj).getType()){
					...
					case EQUIVALENCY:
					case NONEQUIVALENCY:
					if(i<1||!(list.get(i-1) instanceof ScriptValue_Abstract)){throw new Exception_Nodeable_UnexpectedType((Referenced)list.get(i),"Variable");}
					i--;left=(ScriptValue_Abstract)list.get(i);
					...
					i--;left=(ScriptValue_Abstract)list.get(i);
					list.remove(i);list.remove(i);
					if(!(list.get(i)instanceof ScriptLine)){throw new Exception_Nodeable_UnexpectedType((Referenced)list.get(i),"Function name");}
					ScriptLine name=(ScriptLine)list.get(i);list.remove(i);
					if(list.get(i)instanceof ScriptGroup){
					...
					case ASSIGNMENT:
					i--;
					if(!(list.get(i) instanceof ScriptExecutable)){throw new Exception_Nodeable_UnexpectedType((Referenced)list.get(i),"Variable name");}
					...
					}else{returnValue=new ScriptExecutable_AssignValue((Referenced)lhs,lhs,(ScriptValue_Abstract)parseExpression(env,list,automaticallyAddToStack,type));}
					...
					case DIVIDE:
					case MODULUS:
					if(i==0||!(list.get(0) instanceof ScriptValue_Abstract)){throw new Exception_Nodeable_UnexpectedType((Referenced)list.get(0),"Variable");}
					...
					case DIVIDEEQUALS:
					case MODULUSEQUALS:
					if(i==0||!(list.get(0) instanceof ScriptExecutable)){throw new Exception_Nodeable_UnexpectedType((Referenced)list.get(0),"Variable");}
					...
					}else{returnValue=new ScriptExecutable_EvalAssignMathExpression((Referenced)lhs,lhs,(ScriptValue_Abstract)parseExpression(env,list,automaticallyAddToStack,type),((ScriptOperator)obj).getType());}
					...
					if(i>0&&list.get(i-1)instanceof ScriptValue_Abstract){
						// Post-increment
						i--;returnValue=new ScriptExecutable_AutoMathematicator((Referenced)list.get(i+1),(ScriptValue_Abstract)list.get(i),((ScriptOperator)obj).getType(),true);
			...
			if(obj instanceof ScriptLine){
				...
				if(nextObj instanceof ScriptGroup){
					if(i+1==list.size()-1||!(list.get(i+2) instanceof ScriptGroup)){
						...
						list.remove(i);list.remove(i);list.add(i,fxnCall);
				...
				if(nextObj instanceof ScriptLine){
					...
					if(env.retrieveVariable(((ScriptLine)nextObj).getString())!=null){throw new Exception_Nodeable_VariableAlreadyDefined((Referenced)nextObj,null,((ScriptLine)nextObj).getString());}
					ScriptKeywordType permission=ScriptKeywordType.PRIVATE;
					boolean isStatic=false;
					...
					list.remove(i);list.remove(i);list.add(i,creator);
					return parseExpression(env,list,automaticallyAddToStack,type);
				}
			}
		}
	...
	public static List<ScriptValue_Abstract>parseParamGroup(ScriptEnvironment env, ScriptGroup group,ScriptValueType type)throws Exception_Nodeable{return parseParamGroup(env,group.getElements(),type);}
	public static List<ScriptValue_Abstract>parseParamGroup(ScriptEnvironment env, List<Object>elementsList,ScriptValueType type)throws Exception_Nodeable{
		...
		while(iter.hasNext()){
			...
			if(obj instanceof ScriptOperator&&((ScriptOperator)obj).getType()==ScriptOperatorType.COMMA){
				...
				currentParamList.clear();continue;
			}
	...
	public static List<Object>extractNumbers(List<Object>lineList){
		for(int i=0;i<lineList.size();i++){
			...
			if(!(lineList.get(i) instanceof ScriptLine)){continue;}
			...
			if(!number.matches("^[0-9]+[fd]{0,1}$")){continue;}
			...
			if(number.length()>3&&number.substring(number.length()-2).equals("em")){
				...
				lineList.remove(i);lineList.remove(i);
			...
			if(number.indexOf(".")!=-1){
				...
				lineList.remove(i);lineList.add(i,scriptDouble);continue;
			}
			if(number.length()<5){
			...
			if(number.length()<10){
				...
				lineList.remove(i);lineList.add(i,scriptInt);continue;
			}
	...
	public static List<Object>createQuotedElements(ScriptLine line) throws Exception_Nodeable{
		...
		// We've found a string element
			...
			do{
				...
				if(nextStringElem==-1){throw new Exception_Nodeable_UnenclosedStringLiteral(line);}
		...
		// We found a character-string element
			...
			if(nextCharElem==-1){throw new Exception_Nodeable_UnenclosedStringLiteral(line);}
	...
	public static List<Object>removeSingleLineGroupings(List<Object>lineList, String openChar, String closingChar, ScriptGroup.GroupType type, boolean recurse){
		for(int i=0;i<lineList.size();i++){
			if(lineList.get(i) instanceof ScriptGroup){
				if(recurse){((ScriptGroup)lineList.get(i)).setElements(removeSingleLineGroupings(((ScriptGroup)lineList.get(i)).getElements(),openChar,closingChar,type,recurse));}
				continue;
			}
			if(!(lineList.get(i) instanceof ScriptLine)){continue;}
	...
	public static List<Object>removeSingleLineGroupings(ScriptLine line, String openChar, String closingChar, ScriptGroup.GroupType type, boolean recurse){
		...
		while(true){
			...
			if(endGroup==-1){
				List<Object>list=new LinkedList<Object>();list.add(line);
				return list;
			}
	...
	public static List<Object>createGroupings(List<Object>stringList, String openChar, String closingChar, ScriptGroup.GroupType type, boolean recurse) throws Exception_Nodeable{
		...
		for(int i=0;i<stringList.size();i++){
			...
			if(!(obj instanceof ScriptLine)){continue;}
			...
			for(int q=i-1;q>=0;q--){
				if(!(stringList.get(q) instanceof ScriptLine)){newList.add(stringList.get(q));stringList.remove(q);i--;continue;}
				ScriptLine backwardScriptLine=(ScriptLine)stringList.get(q);
				int x=backwardScriptLine.getString().lastIndexOf(openChar);
				if(x==-1){
					if(q==0){throw new Exception_Nodeable_UnenclosedBracket(scriptLine);}
		...
		if(!foundGroup){assert Debugger.closeNode("No group found - naturally returning");}
		return stringList;
	}
	...
	public static List<Object>splitByWhitespace(ScriptLine line){
		...
		if(nextWord.length()>0){list.add(new ScriptLine(nextWord,line,offset));}
	...
	public static List<Object>splitByWhitespace(List<Object>list){
		...
		for(int i=0;i<list.size();i++){
			Object obj=list.get(i);
			if(obj instanceof ScriptGroup){((ScriptGroup)obj).setElements(splitByWhitespace(((ScriptGroup)obj).getElements()));continue;}
			if(!(obj instanceof ScriptLine)){continue;}
			list.remove(i);
	...
	public static List<Object>parseOperators(List<Object>list){
		for(int i=0;i<list.size();i++){
			...
			if(element instanceof ScriptGroup){((ScriptGroup)element).setElements(parseOperators(((ScriptGroup)element).getElements()));continue;}
			if(!(element instanceof ScriptLine)){continue;}
			list.remove(i);
	...
	public static List<Object>removeComments(List<Object>stringList){
		...
		for(Object element:stringList){
			if(!(element instanceof ScriptLine)){
				if(!isParagraphCommenting){list.add(element);}
				continue;
			}
	...
	public static String getString(Object obj)throws Exception_Nodeable{return (String)convert(null,obj);}
	public static Boolean getBoolean(Object obj)throws Exception_Nodeable{return (Boolean)convert(null,obj);}
	public static Number getNumber(Object obj)throws Exception_Nodeable{return (Number)convert(null,obj);}
	...
	public static GraphicalElement getGraphicalElement(Object obj)throws Exception_Nodeable{return (GraphicalElement)convert(null,obj);}
	public static List<ScriptValue_Abstract>getList(Object obj)throws Exception_Nodeable{return ((FauxTemplate_List)getCoreValue(obj)).getList();}
	public static DiscreteRegion getDiscreteRegion(Object obj)throws Exception_Nodeable{return (DiscreteRegion)convert(null,obj);}
	public static Point getPoint(Object obj)throws Exception_Nodeable{return (Point)convert(null,obj);}
	public static InterfaceElement getElement(Object obj)throws Exception_Nodeable{return (InterfaceElement)convert(null,obj);}
	public static Object getObject(Object obj)throws Exception_Nodeable{return convert(null,obj);}
	public static ScriptTemplate_Abstract getTemplate(Object obj)throws Exception_Nodeable{return (ScriptTemplate_Abstract)((ScriptValue_Abstract)obj).getValue();}
	public static Terrestrial getTerrestrial(Object obj)throws Exception_Nodeable{return (Terrestrial)convert(null,obj);}
	public static Scheduler getScheduler(Object obj)throws Exception_Nodeable{return (Scheduler)convert(null,obj);}
	public static Asset getAsset(Object obj)throws Exception_Nodeable{return (Asset)convert(null,obj);}
	public static ScriptTemplate_Abstract getSchedulerListener(Object obj)throws Exception_Nodeable{return (ScriptTemplate_Abstract)getCoreValue(obj);}
	public static Scenario getScenario(Object obj)throws Exception_Nodeable{return (Scenario)convert(null,obj);}
	public static Color getColor(Object obj)throws Exception_Nodeable{return (Color)convert(null,obj);}
	// Engine->Script Conversion functions
	public static FauxTemplate_Ace getRiffAce(Object obj)throws Exception_Nodeable{return (FauxTemplate_Ace)convert(null,obj);}
	public static Ace getAce(Object obj)throws Exception_Nodeable{return (Ace)convert(null,obj);}
	public static FauxTemplate_Archetype getRiffArchetype(Object obj)throws Exception_Nodeable{return (FauxTemplate_Archetype)convert(null,obj);}
	public static Archetype getArchetype(Object obj)throws Exception_Nodeable{return (Archetype)convert(null,obj);}
	public static Stylesheet getRiffStylesheet(Object obj)throws Exception_Nodeable{return (Stylesheet)convert(null,obj);}
	public static Stylesheet getStylesheet(Object obj)throws Exception_Nodeable{return (Stylesheet)convert(null,obj);}
	public static FauxTemplate_InterfaceElement getRiffElement(ScriptEnvironment env,InterfaceElement elem)throws Exception_Nodeable{return (FauxTemplate_InterfaceElement)convert(env,elem);}
	public static FauxTemplate_Asset getRiffAsset(ScriptEnvironment env,Asset asset)throws Exception_Nodeable{return (FauxTemplate_Asset)convert(env,asset);}
	public static ScriptValue_Numeric getRiffNumber(ScriptEnvironment env, Number value)throws Exception_Nodeable{return (ScriptValue_Numeric)convert(env,value);}
	public static ScriptValue_Numeric getRiffDouble(ScriptEnvironment env, double value)throws Exception_Nodeable{return (ScriptValue_Numeric)convert(env,value);}
	public static ScriptValue_Numeric getRiffFloat(ScriptEnvironment env, float value)throws Exception_Nodeable{return (ScriptValue_Numeric)convert(env,value);}
	public static ScriptValue_Numeric getRiffLong(ScriptEnvironment env, long value)throws Exception_Nodeable{return (ScriptValue_Numeric)convert(env,value);}
	public static ScriptValue_Numeric getRiffInt(ScriptEnvironment env, int value)throws Exception_Nodeable{return (ScriptValue_Numeric)convert(env,value);}
	public static ScriptValue_Numeric getRiffShort(ScriptEnvironment env, short value)throws Exception_Nodeable{return (ScriptValue_Numeric)convert(env,value);}
	public static ScriptValue_Boolean getRiffBoolean(ScriptEnvironment env, boolean value)throws Exception_Nodeable{return (ScriptValue_Boolean)convert(env,value);}
	public static ScriptValue_String getRiffString(ScriptEnvironment env, String value)throws Exception_Nodeable{return (ScriptValue_String)convert(env,value);}
	public static FauxTemplate_Point getRiffPoint(ScriptEnvironment env, Object obj)throws Exception_Nodeable{return (FauxTemplate_Point)convert(env,obj);}
	public static FauxTemplate_List getRiffList(ScriptEnvironment env,List<ScriptValue_Abstract>list)throws Exception_Nodeable{return (FauxTemplate_List)convert(env,list);}
	public static FauxTemplate_Color getRiffColor(ScriptEnvironment env,Object obj)throws Exception_Nodeable{return (FauxTemplate_Color)convert(env,obj);}
	public static FauxTemplate_Color getRiffColor(ScriptEnvironment env,Color color)throws Exception_Nodeable{return (FauxTemplate_Color)convert(env,color);}
	public static FauxTemplate_Terrestrial getRiffTerrestrial(ScriptEnvironment env,Terrestrial terrestrial)throws Exception_Nodeable{return (FauxTemplate_Terrestrial)convert(env,terrestrial);}
	public static FauxTemplate_Scheduler getRiffScheduler(ScriptEnvironment env,Scheduler scheduler)throws Exception_Nodeable{return (FauxTemplate_Scheduler)convert(env,scheduler);}
	public static FauxTemplate_Path getRiffPath(ScriptEnvironment env,Point_Path path)throws Exception_Nodeable{return (FauxTemplate_Path)convert(env,path);}
	public static FauxTemplate_DiscreteRegion getRiffDiscreteRegion(ScriptEnvironment env,DiscreteRegion region)throws Exception_Nodeable{return (FauxTemplate_DiscreteRegion)convert(env,region);}
	// Conversion assistance functions
	public static ScriptValue_Abstract getCoreValue(Object obj)throws Exception_Nodeable{return ((ScriptValue_Abstract)obj).getValue();}
	public static Object convert(ScriptEnvironment env,Object object)throws Exception_Nodeable{
		if(object instanceof ScriptConvertible){return convert(env,(ScriptConvertible)object);}
		return convert(env,(ScriptConvertible)((ScriptValue_Abstract)object).getValue());
	}
	public static Object convert(ScriptEnvironment env,ScriptConvertible object)throws Exception_Nodeable{return object.convert();}
	// Generic-conversion fxns
	public static Object convert(ScriptEnvironment env,Number num)throws Exception_Nodeable{return new ScriptValue_Numeric(env,num);}
	public static Object convert(ScriptEnvironment env,boolean value)throws Exception_Nodeable{return new ScriptValue_Boolean(env,value);}
	public static Object convert(ScriptEnvironment env,short num)throws Exception_Nodeable{return new ScriptValue_Numeric(env,new Short(num));}
	public static Object convert(ScriptEnvironment env,int num)throws Exception_Nodeable{return new ScriptValue_Numeric(env,new Integer(num));}
	public static Object convert(ScriptEnvironment env,long num)throws Exception_Nodeable{return new ScriptValue_Numeric(env,new Long(num));}
	public static Object convert(ScriptEnvironment env,float num)throws Exception_Nodeable{return new ScriptValue_Numeric(env,new Float(num));}
	public static Object convert(ScriptEnvironment env,double num)throws Exception_Nodeable{return new ScriptValue_Numeric(env,new Double(num));}
	public static Object convert(ScriptEnvironment env,String string)throws Exception_Nodeable{return new ScriptValue_String(env,string);}
	...
	public static String printParseList(List<Object>list, int offset){
		...
		for(int i=0;i<list.size();i++){
			Object obj=list.get(i);
			if(obj instanceof ScriptGroup){string+=RiffToolbox.tab(offset)+"Script Group:\n"+printParseList(((ScriptGroup)obj).getElements(),offset+1);continue;}
