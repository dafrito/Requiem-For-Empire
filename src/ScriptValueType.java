...
public class ScriptValueType{
	...
	public ScriptKeywordType getKeywordType(){
		if(equals(VOID)){return ScriptKeywordType.VOID;}
		if(equals(BOOLEAN)){return ScriptKeywordType.BOOLEAN;}
		if(equals(SHORT)){return ScriptKeywordType.SHORT;}
		if(equals(INT)){return ScriptKeywordType.INT;}
		if(equals(LONG)){return ScriptKeywordType.LONG;}
		if(equals(FLOAT)){return ScriptKeywordType.FLOAT;}
		if(equals(DOUBLE)){return ScriptKeywordType.DOUBLE;}
		if(equals(STRING)){return ScriptKeywordType.STRING;}
		return null;
	}
	...
	public static ScriptValueType getObjectType(ScriptEnvironment env){return createType(env,FauxTemplate_Object.OBJECTSTRING);}
	...
	public ScriptValueType getBaseType()throws Exception_Nodeable{return this;}
	public int getType()throws Exception_Nodeable{
		if(getBaseType()==this){return m_type;
		}else{return getBaseType().getType();}
	}
	public static boolean isConvertibleTo(ScriptEnvironment env,ScriptValueType base,ScriptValueType cast){
		if(isNumericType(base)){return ScriptValue_Numeric.testNumericValueConversion(base,cast);}
		if(isPrimitiveType(base)){return base.equals(cast);}
	...
	public static ScriptValueType getType(Number number){
		if(number instanceof Integer){return ScriptValueType.INT;}
		if(number instanceof Double){return ScriptValueType.DOUBLE;}
		if(number instanceof Short){return ScriptValueType.SHORT;}
		if(number instanceof Long){return ScriptValueType.LONG;}
		if(number instanceof Float){return ScriptValueType.FLOAT;}
		throw new Exception_InternalError("Invalid default");
	}
	public static boolean isNumericType(ScriptValueType type){
		if(type.equals(ScriptValueType.SHORT)){return true;}
		if(type.equals(ScriptValueType.INT)){return true;}
		if(type.equals(ScriptValueType.LONG)){return true;}
		if(type.equals(ScriptValueType.FLOAT)){return true;}
		if(type.equals(ScriptValueType.DOUBLE)){return true;}
		return false;
	}
	public boolean isPrimitiveType(){return isPrimitiveType(this);}
	...
	public static boolean isStringType(ScriptValueType type){return type.equals(ScriptValueType.STRING);}
	public static boolean isBooleanType(ScriptValueType type){return type.equals(ScriptValueType.BOOLEAN);}
	public static boolean isReturnablePrimitiveType(ScriptValueType type){return (isPrimitiveType(type)||type.equals(ScriptValueType.VOID));}
