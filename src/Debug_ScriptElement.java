...
public class Debug_ScriptElement extends JPanel implements UndoableEditListener,ListSelectionListener,ComponentListener,MouseListener{
	...
	public Debug_ScriptElement(Debug_Environment debugger){
		m_debugger=debugger;
		if(selectFile(false)){m_isValid=openFile();}
	}
	public Debug_ScriptElement(Debug_Environment debugger,String string){
		m_debugger=debugger;
		if(string!=null){m_file=new File(string);}
		m_isValid=openFile();
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(m_errors.getSelectedValue()==null){return;}
		Exception rawEx=m_exceptions.get(m_errors.getSelectedIndex());
		if(!(rawEx instanceof Exception_Nodeable)){return;}
		...
		for(int i=0;i<ex.getLineNumber();i++){
			if(array.length-1==i){m_textArea.setCaretPosition(offset);return;}
	...
	public boolean canUndo(){return m_edits.size()>0;}
	public boolean canRedo(){return m_undoneEdits.size()>0;}
	public boolean hasChanged(){return m_hasChanged;}
	public void undo(){
		setChanged(true);
		m_edits.peek().end();
		if(m_edits.size()>0){
			...
			if(m_edits.size()==0){m_debugger.setCanUndo(false);}
	...
	public void redo(){
		...
		if(m_undoneEdits.size()>0){
			...
			edit.redo();
			m_edits.push(edit);
			if(m_undoneEdits.size()==0){m_debugger.setCanRedo(false);}
			m_debugger.setCanUndo(true);
		}
	}
	...
	public String getName(){return m_prefix+getFilename();}
	public String getFilename(){
		if(m_file!=null){return m_file.getName();}
	...
	public boolean openFile(){return openFile(m_file);}
	...
	public boolean saveFile(){
		...
		if(m_file==null){return saveFileAs();}
		return writeFile();
	}
	public boolean saveFileAs(){
		m_debugger.showReferenced(this);
		if(!selectFile(true)){return false;}
	...
	public boolean selectFile(boolean isSaving){
		...
		int choice=0;
		if(isSaving){choice=fileChooser.showSaveDialog(this);
		}else{choice=fileChooser.showOpenDialog(this);}
	...
	public boolean readFile(){return readFile(m_file);}
	...
	public boolean writeFile(){return writeFile(m_file);}
