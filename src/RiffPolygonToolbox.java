...
public class RiffPolygonToolbox{
	...
	public static boolean isPolygonConvex(DiscreteRegion region){
		...
		for(int i=0;i<pointList.size();i++){
			...
			for(int k=0;k<pointList.size();k++){
				...
				if(myBool == null){
					if(value>0){myBool=new Boolean(true);
					}else if(value<0){myBool=new Boolean(false);}
				}else{
					if(value > 0){
	...
	public static DiscreteRegion optimizePolygon(DiscreteRegion region){
		if(region.isOptimized()){return region;}
		...
		for(int i=0;i<pointList.size();i++){
			for(int j=0;j<pointList.size();j++){
				...
				if(point==null||point.isTangent()){continue;}
	...
	public static int getCrosses(double xCoord, double yCoord, DiscreteRegion region){
		...
		for(int i=0;i<pointList.size();i++){
			...
			if(intersect==null||intersect.isTangent()){continue;}
			crosses++;
		}
	...
	public static boolean getBoundingRectIntersection(Point pointA, Point pointB, Point pointC, Point pointD, boolean includeTangents){
		if(pointA.equals(pointC)&&pointB.equals(pointD)){return true;}
		if(pointA.equals(pointD)&&pointB.equals(pointC)){return true;}
		if(RiffToolbox.areEqual(Point.System.EUCLIDEAN, Math.abs(RiffPolygonToolbox.getSlope(pointA, pointB)),Double.POSITIVE_INFINITY)&&RiffToolbox.areEqual(Point.System.EUCLIDEAN, Math.abs(RiffPolygonToolbox.getSlope(pointC,pointD)),Double.POSITIVE_INFINITY)){
			if(RiffToolbox.areEqual(pointA, pointA.getX(),pointC.getX())){return false;}
		}else{
			if(RiffToolbox.isLessThan(Math.max(pointA.getX(), pointB.getX()),  Math.min(pointC.getX(), pointD.getX()))){return false;}
			if(RiffToolbox.areEqual(pointA, Math.max(pointA.getX(), pointB.getX()),  Math.min(pointC.getX(), pointD.getX()))&&!includeTangents){return false;}
			if(RiffToolbox.isGreaterThan(Math.min(pointA.getX(), pointB.getX()), Math.max(pointC.getX(), pointD.getX()))){return false;}
			if(RiffToolbox.areEqual(pointA, Math.min(pointA.getX(), pointB.getX()), Math.max(pointC.getX(), pointD.getX()))&&!includeTangents){return false;}
		}
		if(RiffToolbox.areEqual(pointA, RiffPolygonToolbox.getSlope(pointA, pointB),0.0d)&&RiffToolbox.areEqual(Point.System.EUCLIDEAN, RiffPolygonToolbox.getSlope(pointC,pointD),0.0d)){
			if(!RiffToolbox.areEqual(pointA, pointA.getY(),pointC.getY())){return false;}
		}else{
			if(RiffToolbox.isLessThan(Math.max(pointA.getY(), pointB.getY()), Math.min(pointC.getY(), pointD.getY()))){return false;}
			if(RiffToolbox.areEqual(pointA, Math.max(pointA.getY(), pointB.getY()), Math.min(pointC.getY(), pointD.getY()))&&!includeTangents){return false;}
			if(RiffToolbox.isGreaterThan(Math.min(pointA.getY(), pointB.getY()), Math.max(pointC.getY(), pointD.getY()))){return false;}
			if(RiffToolbox.areEqual(pointA, Math.min(pointA.getY(), pointB.getY()), Math.max(pointC.getY(), pointD.getY()))&&!includeTangents){return false;}
		}
		return true;
	}
	...
	public static DiscreteRegionBSPNode removeOverlappingPolygons(DiscreteRegionBSPNode root, DiscreteRegion region, boolean recurse){
		...
		while(iter.hasNext()){
			...
			if(region.checkClearedRegionMap(otherRegion)){
				...
				potentialList.remove(otherRegion);iter=potentialList.iterator();
				continue;
			}
	...
	public static boolean testForColinearity(Point pointA, Point pointB, Point testPointA, Point testPointB){
		...
		if((pointA.equals(testPointA)&&pointB.equals(testPointB))||(pointA.equals(testPointB)&&pointB.equals(testPointA))){return true;}
			...
			for(int j=0;j<polyList.size();j++){
				...
				for(int q=0;q<firstRegion.getPoints().size();q++){
					for(int x=0;x<secondRegion.getPoints().size();x++){
						...
						if(!RiffPolygonToolbox.isPolygonConvex(testRegion)){continue;}
	...
	public static boolean getBoundingRectIntersection(double xMin, double xMax, double yMin, double yMax, Point point, boolean allowTangent){
		if(RiffToolbox.isLessThan(point.getX(), xMin)){return false;}
		if(RiffToolbox.areEqual(point,point.getX(), xMin)&&!allowTangent){return false;}
		if(RiffToolbox.isGreaterThan(point.getX(), xMax)){return false;}
		if(RiffToolbox.areEqual(point,point.getX(), xMax)&&!allowTangent){return false;}
		if(RiffToolbox.isLessThan(point.getY(), yMin)){return false;}
		if(RiffToolbox.areEqual(point,point.getY(), yMin)&&!allowTangent){return false;}
		if(RiffToolbox.isGreaterThan(point.getY(), yMax)){return false;}
		if(RiffToolbox.areEqual(point,point.getY(), yMax)&&!allowTangent){return false;}
		return true;
	}
	...
	public static void doPointSideTest(PointSideStruct struct,Point point,double value){
		...
		}else{struct.addLeft(point,value);}
	}
...
class RiffIntersectionPoint implements Nodeable{
	...
	public Point getIntersection(){return m_intersect;}
	...
	public boolean equals(Object o){
		if(!(o instanceof RiffIntersectionPoint)){return false;}
...
class PointSideStruct implements Nodeable{
	...
	public PointSideStruct(){
		m_left=new LinkedList<Point>();
		m_right=new LinkedList<Point>();
		m_indeterminates=new LinkedList<Point>();
	}
	public PointSideStruct(List<Point>left,List<Point>right,List<Point>indet){
		m_left=left;
		m_right=right;
		m_indeterminates=indet;
	}
	public void addLeft(Point point,double value){m_left.add(point);m_leftWeight+=value;}
	public void addRight(Point point,double value){m_right.add(point);m_rightWeight+=value;}
	public void addIndeterminate(Point point){m_indeterminates.add(point);}
	public List<Point> getLeftPoints(){return m_left;}
	public List<Point> getRightPoints(){return m_right;}
	public List<Point> getIndeterminates(){return m_indeterminates;}
	public boolean isStraddling(){return m_left.size()!=0&&m_right.size()!=0;}
	public boolean isLessThan(){return m_left.size()!=0&&m_right.size()==0;}
	public boolean isGreaterThan(){return m_left.size()==0&&m_right.size()!=0;}
	public boolean hasIndeterminates(){return !m_indeterminates.isEmpty();}
	public boolean isColinear(){return m_left.size()==0&&m_right.size()==0;}
	public void validate(){
		if(Math.abs(m_rightWeight)<Math.pow(10, -5)&&m_right.size()>0){
			assert Debugger.addNode("Right-side list is insignificant, clearing.");
			m_right.clear();
		}
		if(Math.abs(m_leftWeight)<Math.pow(10, -5)&&m_left.size()>0){
	...
	public boolean nodificate(){
		...
		assert Debugger.addSnapNode("Left-side Points (Weight: " + m_leftWeight+"): " + m_left.size() + " element(s)",m_left);
		assert Debugger.addSnapNode("Right-side Points (Weight: " + m_rightWeight+"): " + m_right.size() + " element(s)",m_right);
		assert Debugger.addSnapNode("Indeterminate Points: " + m_indeterminates.size() + " element(s)",m_indeterminates);
		assert Debugger.closeNode();
		return true;
	}
}
